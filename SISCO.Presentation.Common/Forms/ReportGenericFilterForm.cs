...
namespace SISCO.Presentation.Common.Forms
{
    public partial class ReportGenericFilterForm<T> : Form
    {
        ...
        public TextExportMode TextMode { get; set; }

        public ReportGenericFilterForm()
        {
            InitializeComponent();
            TextMode = TextExportMode.Text;
            ...
            Load += (sender, args) =>
            {
                ...
                foreach (var key in Parameters.Keys)
                {
                    ...
                    if (control.GetType() == typeof (dLookup))
                    {
                        ((dLookup) control).Properties.ButtonClick += (o, eventArgs) =>
                        {
                            var dlookup = o as dLookup;
