...
{
    public partial class DataEntryEcommerceForm : BaseCrudForm<ShipmentModel, ShipmentDataManager>//BaseToolbarForm//
    {
        private bool EditCustomer { get; set; }
        public bool isAwbAvailable { get; set; }
        private bool isBulk { get; set; }
        private BaseForm bulkForm { get; set; }

        public DataEntryEcommerceForm()
        {
            InitializeComponent();
            form = this;
        ...
        private bool CheckCn()
        {
            if (tbxNo.Text != "")
            {
                ...
                var allocation = new ShipmentNumberAllocationDataManager().GetAllocationByCustomer(Convert.ToInt64(tbxNo.Text), BaseControl.BranchId);
                if (allocation != null)
                {
                    ...
                    if (allocation.FranchiseId != null)
                    {
                        var code = tbxNo.Text;

                        New();

                        MessageForm.Warning(form, "POD Cash", string.Format("POD {0} sudah dialokasi untuk agent Franchise. Silakan info ke Counter Cash", code));
                        return false;
                    }
                }
            }
        ...
        public void OpenPopulateForm(ShipmentModel model, BaseForm form)
        {
            New();
            ...
            btnSave.Enabled = true;
            isBulk = true;
            cbxPrinteconnote.Checked = expand.PrintEconnote;

            if (tbxNo.Text != string.Empty) tbxDestination.Focus();
            tbxW.SetValue((decimal)1);
            tbxL.SetValue((decimal)1);
            tbxH.SetValue((decimal)1);

            tbxMessenger.DefaultValue = new IListParameter[] { WhereTerm.Default(BaseControl.EmployeeId, "id", EnumSqlOperator.Equals) };
        }
