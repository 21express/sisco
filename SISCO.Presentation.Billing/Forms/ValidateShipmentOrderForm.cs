...
{
    public partial class ValidateShipmentOrderForm : BaseCrudForm<ShipmentModel, ShipmentDataManager>//BaseToolbarForm//
    {
        ...
        private LookupModel lookup { get; set; }
        ...
        public ValidateShipmentOrderForm()
        {
            InitializeComponent();
            form = this;
        }
        ...
        protected override T LoadModel<T>(params IListParameter[] listParameters)
        {
            ...
            var model =
                ((ShipmentDataManager)DataManager).GetCreditCollectOnly<T>(BaseControl.BranchId, Paging.Instance(CurrentIndexPage - 1, PageLimit), out count, listParameters)
                    .FirstOrDefault();
            TotalRecord = count;
            return model;
        }
        ...
        protected override void LoadForm(object sender, EventArgs e)
        {
            ...
            lookup = new LookupDataManager().Get("MinInsurance");
            ...
            tbxMessenger2.LookupPopup = new EmployeePopup(EmployeeModel.EmployeeType.Messenger);
            ...
            txtOtherLabel.FieldLabel = "Other Title";
            txtOtherLabel.ValidationRules = new[] { new ComponentValidationRule(EnumComponentValidationRule.Mandatory) };
            ...
            txtOtherLabel.CharacterCasing = CharacterCasing.Normal;
            ...
            {
                if (tbxRate.Value > 0 && txtGoodsValue.Value > 0)
                {
                    var insurance = Math.Round(txtGoodsValue.Value * ((decimal)tbxRate.Value / 100), MidpointRounding.AwayFromZero);
                    txtInsuranceFee.SetValue(insurance);
                    int temp;
                    if (int.TryParse(lookup.Value, out temp))
                    {
                        if (temp > insurance) txtInsuranceFee.SetValue(Convert.ToDecimal(temp));
                    }
            ...
            txtDeliveryTariff1.KeyUp += (o, args) => { if (Convert.ToInt32(args.KeyCode) != 13) RefreshDeliveryTariff(); };

            txtDeliveryTariff1.KeyUp += (o, args) => { if (Convert.ToInt32(args.KeyCode) != 13) RefreshDeliveryTariff(); };

            lkpBranch.EditValueChanged += (o, args) => RefreshServiceType();
        ...
        void lkpCustomer_TextChanged(object sender, EventArgs e)
        {
            ...
            if (((ShipmentModel)CurrentModel).Voided || ((ShipmentModel)CurrentModel).Invoiced == 1) return;
        ...
        protected override void PopulateForm(ShipmentModel model)
        {
            ...
            tbxMessenger2.DefaultValue = new IListParameter[] { WhereTerm.Default(model.MessengerId2 ?? 0, "id") };
            ...
            txtOtherLabel.Text = expand != null ? !string.IsNullOrEmpty(expand.OtherLabel) ? expand.OtherLabel : "Other" : "Other";
            ...
            lkpPaymentMethod.Enabled = false;
        ...
        protected override ShipmentModel PopulateModel(ShipmentModel model)
        {
            ...
            model.MessengerName2 = tbxMessenger2.Text;
        ...
        protected override bool ValidateFormWithAlert()
        {
            ...
            if (lkpCustomer.Value == null && ((ShipmentModel)CurrentModel).SalesTypeId != 3)
            {
            ...
            if (txtOtherLabel.Text == string.Empty)
            {
                MessageBox.Show("Please enter the title of other fee!");
                return false;
            }
            if (lkpPackageType.Value == null)
            {
            ...
            if (txtChargeableWeight.Value == 0)
            {
                MessageBox.Show("Please enter the chargeable weight!");
                return false;
            }
            if (lkpPaymentMethod.Text.ToUpper().Equals("COLLECT") && lkpCustomerCollect.Value == null)
            {
                MessageBox.Show("Silahkan pilih customer collect!");
                return false;   
            }
            // ReSharper restore LocalizableElement

            if (txtGoodsValue.Value > 0 && tbxRate.Value == 0)
            {
                MessageBox.Show("Silakan masukkan rate untuk asuransi.");
                tbxRate.Focus();
                return false;
            }
        ...
        protected override ShipmentModel Find(string searchTerm)
        {
            ...
            int count;
            return ((ShipmentDataManager)DataManager).GetCreditCollectOnly<ShipmentModel>(BaseControl.BranchId, Paging.Instance(0, 1), out count, WhereTerm.Default(searchTerm, "code"))
                    .FirstOrDefault();
        }
        ...
        private void RefreshTariff(bool force = false)
        {
            ...
            if (((ShipmentModel)CurrentModel).Invoiced == 1 || ((ShipmentModel)CurrentModel).Voided) return;
        ...
        protected override void RefreshToolbarState()
        {
            ...
            if (model != null && (model.Voided || model.Invoiced == 1))
            {
