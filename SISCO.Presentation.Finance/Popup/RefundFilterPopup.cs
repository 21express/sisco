...
{
    public partial class RefundFilterPopup : BaseSearchCode<RefundPph23Model, RefundPph23DataManager>//BaseSearchPopup//
    {
        public RefundFilterPopup()
        {
            InitializeComponent();
        ...
        {
            var param = new List<WhereTerm>();
            var dm = new RefundPph23DataManager();

            if (tbxStt.Text != "")
            {
                param.Add(WhereTerm.Default(tbxStt.Text, "invoice_ref_number", EnumSqlOperator.Like));
            }
