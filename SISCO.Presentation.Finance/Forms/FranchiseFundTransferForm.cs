...
{
    public partial class FranchiseFundTransferForm : BaseCrudForm<FranchiseFundTransferModel, FranchiseFundTransferDataManager>//BaseToolbarForm//
    {
        public FranchiseFundTransferForm()
        {
            InitializeComponent();
            form = this;
            ...
            Load += FranchiseFundTransferLoad;
        ...
        private void FranchiseFundTransferLoad(object sender, EventArgs e)
        {
            ClearForm();
            EnabledForm(false);
            EnableBtnSearch = true;
            SearchPopup = new FranchiseFundTransferFilterPopup();
        ...
        {
            var unpaid= new FranchisePaymentInDataManager().GetUnpaidFranchisePayment(BaseControl.BranchId);
        ...
        {
            var obj = GridPayment.DataSource as List<FranchiseFundTransferDetailModel>;
            if (obj != null)
        ...
        {
            Enabled = false;
            var dm = new FranchiseFundTransferDetailDataManager();
            var fpi = new FranchisePaymentInDataManager();
            for (var i = 0; i < PaymentView.RowCount; i++)
            {
                var state = PaymentView.GetRowCellValue(i, PaymentView.Columns["StateChange"]).ToString();
                var detail = new FranchiseFundTransferDetailModel();
                ...
                if (state == EnumStateChange.Insert.ToString() && (bool)PaymentView.GetRowCellValue(i, PaymentView.Columns["Checked"]))
                {
                    ...
                    detail.FranchiseFundTransferId = CurrentModel.Id;
                    ...
                    var p = fpi.GetFirst<FranchisePaymentInModel>(WhereTerm.Default(detail.InvoiceId, "id"));
                    if (p != null)
                    {
                        p.FundTransfer = true;
                    ...
                    dm.Save<FranchiseFundTransferDetailModel>(detail);
                }
                ...
                if (state == EnumStateChange.Update.ToString() && !(bool)PaymentView.GetRowCellValue(i, PaymentView.Columns["Checked"]))
                {
                    ...
                    var p = fpi.GetFirst<FranchisePaymentInModel>(WhereTerm.Default((int)PaymentView.GetRowCellValue(i, PaymentView.Columns["InvoiceId"]), "id"));
                    if (p != null)
                    {
        ...
        {
            ToolbarCode = ((FranchiseFundTransferModel)CurrentModel).Code;
            OpenData(ToolbarCode);
        }

        public override void AfterDelete()
        {
            var detail = new FranchiseFundTransferDetailDataManager().Get<FranchiseFundTransferDetailModel>(WhereTerm.Default(CurrentModel.Id, "franchise_fund_transfer_id", EnumSqlOperator.Equals));
            if (detail != null)
            {
                var repo = new FranchiseFundTransferDetailDataManager();
                var r = new FranchisePaymentInDataManager();
                foreach (FranchiseFundTransferDetailModel obj in detail)
                {
                    var d = r.GetFirst<FranchisePaymentInModel>(WhereTerm.Default((int)obj.InvoiceId, "id", EnumSqlOperator.Equals));
        ...
        protected override void PopulateForm(FranchiseFundTransferModel model)
        {
            ClearForm();
            if (model == null)
            {
            ...
            GridPayment.DataSource = new FranchiseFundTransferDetailDataManager().Get<FranchiseFundTransferDetailModel>(WhereTerm.Default(model.Id, "franchise_fund_transfer_id"));
            PaymentView.RefreshData();
        ...
        protected override FranchiseFundTransferModel PopulateModel(FranchiseFundTransferModel model)
        {
            model.DateProcess = tbxDate.DateTime;
            model.Code = CurrentModel.Id > 0 ? ((FranchiseFundTransferModel)CurrentModel).Code : GetCode("fundtransfer", tbxDate.DateTime);
        ...
        protected override FranchiseFundTransferModel Find(string searchTerm)
        {
            ...
            var obj = DataManager.GetFirst<FranchiseFundTransferModel>(param);
            PopulateForm(obj);
        ...
        protected override void RefreshToolbarState()
        {
            ...
            var model = CurrentModel as FranchiseFundTransferModel;
