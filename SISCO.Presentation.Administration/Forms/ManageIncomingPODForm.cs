...
{
    public partial class ManageIncomingPodForm : BaseCrudForm<PodInModel, PodInDataManager>, IGridChildForm//BaseToolbarForm//
    {
        protected BindingList<PodInShipmentDataRow> Shipments { get; set; }
        public BranchDataManager BranchDataManager { get; set; }
        public ShipmentDataManager ShipmentDataManager { get; set; }

        public class PodInShipmentDataRow : PodInShipmentModel, INotifyPropertyChanged
        {
            public BranchDataManager BranchDataManager { get; set; }
            public ShipmentDataManager ShipmentDataManager { get; set; }
        ...
        protected override void LoadForm(object sender, EventArgs e)
        {
            ...
            Shipments = new BindingList<PodInShipmentDataRow>();
            ...
            {
                var printout = new TandaTerimaPodKembaliPrintOut
                {
                    ...
                    Parameters =
                    {
                        ...
                        {
                            Name = "DateProcess",
                            Value = ((PodInModel) CurrentModel).Vdate
                        }
                    }
                };
                printout.Print();
        ...
        protected void btnAdd_Click()
        {
            ...
            if (shipmentModel.BranchId != BaseControl.BranchId)
            {
                txtAddShipmentNo.SelectAll();
                MessageBox.Show(@"POD harusnya bukan kembali ke cabang ini!");
                txtAddShipmentNo.Focus();
                txtAddShipmentNo.SelectAll();
                return;
            }
            ...
            var podOutShipmentModel = new PodOutShipmentDataManager().GetFirst<PodOutShipmentModel>(WhereTerm.Default(shipmentModel.Id, "shipment_id"));

            Shipments.Add(new PodInShipmentDataRow
            {
                BranchDataManager = BranchDataManager,
                ShipmentDataManager = ShipmentDataManager,

                PodInId = CurrentModel.Id,
                ...
                Returned = 0,
                Note = "",
                SuratJalan = (podOutShipmentModel != null && podOutShipmentModel.SuratJalan != null ? podOutShipmentModel.SuratJalan : ""),
        ...
        protected override void PopulateForm(PodInModel model)
        {
            ...
            new PodInShipmentDataManager().
                Get<PodInShipmentModel>(WhereTerm.Default(model.Id, "pod_in_id")).
                Select(row => new PodInShipmentDataRow
                {
                    BranchDataManager = BranchDataManager,
                    ShipmentDataManager = ShipmentDataManager,
        ...
        protected override PodInModel PopulateModel(PodInModel model)
        {
        ...
        protected override PodInModel Find(string searchTerm)
        {
            return DataManager.GetFirst<PodInModel>(WhereTerm.Default(searchTerm, "Code"), WhereTerm.Default(BaseControl.BranchId, "branch_id"));
        }
        ...
        public override void Save()
        {
            ...
            CurrentModel = PopulateModel(CurrentModel as PodInModel);

            if (CurrentModel.Id == 0)
            {
                ((PodInModel) CurrentModel).Vtime = DateTime.Now.ToString("HH:mm");
                ...
                ((PodInDataManager)DataManager).Save<PodInModel>(CurrentModel);
            }
            else
            {
                ...
                ((PodInDataManager)DataManager).Update<PodInModel>(CurrentModel);
            }

            new PodInShipmentDataManager().Save(CurrentModel.Id, Shipments);

            tsBaseTxt_Code.Text = ((PodInModel)CurrentModel).Code;
