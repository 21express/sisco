...
namespace SISCO.Presentation.Franchise.Popup
{
    ...
    {
        private FranchiseModel Franchise { get; set; }
        public ShipmentPopup(int franchiseId)
        {
            InitializeComponent();

            Franchise = new FranchiseDataManager().GetFirst<FranchiseModel>(WhereTerm.Default(franchiseId, "id", EnumSqlOperator.Equals));
        ...
        private void OnLoad(object sender, EventArgs e)
        {
            ...
            tbxCustomer.AutoCompleteWhereExpression = (s, p) =>
                p.SetWhereExpression("(code = @0 OR name.StartsWith(@0)) AND branch_id = @1 AND franchise_id = @2", s, BaseControl.BranchId, Franchise.Id);
        ...
        protected override void BeforeFilter()
        {
            ...
            param.Add(WhereTerm.Default(Franchise.BranchId, "branch_id", EnumSqlOperator.Equals));
            param.Add(WhereTerm.Default(Franchise.Id, "franchise_id", EnumSqlOperator.Equals));
            param.Add(WhereTerm.Default(Franchise.CityId, "city_id", EnumSqlOperator.Equals));
