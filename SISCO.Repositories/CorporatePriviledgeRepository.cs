...
{
    public class CorporatePriviledgeRepository : SISCOBaseRepository
    {
        private const string OBJECT_NAME = "CorporatePriviledge";
        public CorporatePriviledgeRepository()
        {
            ObjectName = OBJECT_NAME;
        }

        public CorporatePriviledgeRepository(SISCODBEntities entity)
        {
            ObjectName = OBJECT_NAME;
            Entities = entity;
        }
        ...
        {
            var model = businessModel as CorporatePriviledgeModel;
            if (model == null)
                throw new ModelNullException();
            var entity = PopulateModelToNewEntity(model);
            Entities.AddTocorporate_priviledge(entity);
        ...
        public override void Update<T>(T businessModel)
        {
            ...
            var query = (from d in Entities.corporate_priviledge where d.id == model.Id select d).FirstOrDefault();
            if (query == null)
        ...
        public override void Update<T>(T businessModel, params IListParameter[] parameter)
        {
            ...
            if (model == null)
                throw new ModelNullException(MessageModelNull);
            var query = (from d in Entities.corporate_priviledge select d).Where(whereterm, ListValue.ToArray()).FirstOrDefault();
            if (query == null)
                throw new ModelNullException(MessageEntityNotFound);
            PopulateModelToNewEntity(query, model);
            Entities.SaveChanges();
        ...
        {
            var query = (from d in Entities.corporate_priviledge where d.id == id select d).FirstOrDefault();
            if (query == null)
                throw new ModelNullException(MessageEntityNotFound);
        ...
        private static corporate_priviledge PopulateModelToNewEntity(CorporatePriviledgeModel model)
        {
            return new corporate_priviledge
            {
                id = model.Id,
        ...
        private static void PopulateModelToNewEntity(corporate_priviledge query, CorporatePriviledgeModel model)
        {
            query.id = model.Id;
        ...
        private static CorporatePriviledgeModel PopulateEntityToNewModel(corporate_priviledge item)
        {
            return new CorporatePriviledgeModel
            {
                Id = item.id,
        ...
        {
            var whereterm = GetQueryParameterLinq(parameter);
            var query = (from a in Entities.corporate_priviledge select a).Where(whereterm, ListValue.ToArray()).AsQueryable();
            if (query == null)
        ...
        {
            var whereterm = GetQueryParameterLinq(parameter);
            var query = (from a in Entities.corporate_priviledge select a).Where(whereterm, ListValue.ToArray()).ToList();
            if (query == null)
                throw new Exception(MessageEntityNotFound);
        ...
        {
            var whereterm = GetQueryParameterLinq(parameter);
            var query = (from a in Entities.corporate_priviledge select a).Where(whereterm, ListValue.ToArray()).OrderBy(paging.SortColumn + " " + paging.Direction).Skip(paging.Start).Take(paging.Limit).ToList();
            if (query == null)
                throw new Exception(MessageEntityNotFound);
            var tquery = (from a in Entities.corporate_priviledge select a).Where(whereterm, ListValue.ToArray());
        ...
        {
            var query = from p in Entities.corporate_priviledge
                        from up in Entities.corporate_user_priviledge.Where(r => r.module_name == p.module_name && r.form_name == p.form_name && r.corporate_user_id == corporateUserId && r.rowstatus).DefaultIfEmpty()
                        where p.rowstatus
                        select new CorporatePrivilegeJoinCorporateUserPrivilegeModel
                        {
                            Id = p.id,
                            ...
                            CorporateUserId = up.corporate_user_id,
