...
namespace Franchise.Presentation.Common
{
    public class BaseControl
    {
        ...
        public static int FranchiseId { get; set; }
        public static string FranchiseName { get; set; }
        ...
        public static string UrlAPiFranchise { get { return "api/v1/franchise/validation"; } }
        #else
        private static string BaseAddress { get { return "http://pgcunx-dev/"; } }
        public static string UrlAPiAccessToken { get { return "api/oauth/access_token"; } }
        public static string UrlAPiFranchise { get { return "api/v1/franchise/validation"; } }
        # endif

        private static string _franchiseConfigLocation = "franchiseconfig.xml";
        public static string FranchiseConfigLocation
        {
            get
            {
                if (!ApplicationDeployment.IsNetworkDeployed)
                {
                    return _franchiseConfigLocation;
                }

                var localConnAppsLocation = Path.Combine(ApplicationDeployment.CurrentDeployment.DataDirectory, _franchiseConfigLocation);

                if (!File.Exists(localConnAppsLocation))
                {
                    File.Copy(_franchiseConfigLocation, localConnAppsLocation);
                }
        ...
        public static FranchiseViewModel GetProductKey(string productKey)
        {
            var result = new FranchiseViewModel();
            var client = new HttpClient();
            client.BaseAddress = new Uri(BaseAddress);
            var uri = string.Format("{0}?access_token={1}&product_key={2}", UrlAPiFranchise, AccessToken, HttpUtility.UrlEncode(productKey));
            var task = client.GetAsync(uri).ContinueWith((s) =>
            {
                ...
                var jsonConvert = new JavaScriptSerializer();
                var actionResult = jsonConvert.Deserialize<FranchiseViewModel>(jsonResponse.Result);
                if (actionResult != null)
                {
                    result = actionResult;
                }
