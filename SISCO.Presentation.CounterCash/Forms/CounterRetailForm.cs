...
{
    public partial class CounterRetailForm : BaseCrudForm<ShipmentModel, ShipmentDataManager>//BaseToolbarForm//
    {
        ...
        public CounterRetailForm()
        {
            ...
            DataManager.DefaultParameters = new IListParameter[]
            {
                ...
                WhereTerm.Default(4,"sales_type_id", EnumSqlOperator.Equals)
            };

            Customer = new CustomerDataManager().GetFirst<CustomerModel>(
                WhereTerm.Default("RETAIL", "code", EnumSqlOperator.Like),
                WhereTerm.Default(BaseControl.BranchId, "branch_id", EnumSqlOperator.Equals)
            );
        }
        ...
        protected override void LoadForm(object sender, EventArgs e)
        {
            ...
            SearchPopup = new ShipmentPopup(4);
        ...
        protected override void PopulateForm(ShipmentModel model)
        {
            ...
            if (model.CustomerId != null)
                Customer = new CustomerDataManager().GetFirst<CustomerModel>(WhereTerm.Default((int) model.CustomerId, "id"));
            else Customer = new CustomerDataManager().GetFirst<CustomerModel>(
                WhereTerm.Default("Retail", "name", EnumSqlOperator.Equals),
                WhereTerm.Default(BaseControl.BranchId, "branch_id", EnumSqlOperator.Equals)
            );
            ...
            if (((ShipmentModel) CurrentModel).PricingPlanId == PRICING_PLAN_DOMESTIC)
            {
                var customerTariffModel = new CustomerTariffDataManager().GetTariff(
        ...
        public override void New()
        {
            ...
            txtDate.Enabled = false;
            lkpBranch.Enabled = false;
            txtDiscount.Value = Customer.Discount;
            txtShipmentNo.Enabled = false;
            lkpMessenger.Enabled = false;
            tbxPromo.Enabled = false;
        ...
        private void RefreshServiceType()
        {
            ...
            {
                var originCity = cityDataManager.GetFirst<CityModel>(WhereTerm.Default((int) lkpOriginCity.Value, "id"));
                if (originCity != null)
                {
        ...
        bool CheckCn()
        {
            var ship = DataManager.GetFirst<ShipmentModel>(WhereTerm.Default(txtShipmentNo.Text, "code", EnumSqlOperator.Equals));
            if (ship != null)
            {
